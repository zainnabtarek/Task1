...
def draw_status(score,type,exp,gene,used_pieces):
    """Draw status"""

    # type of running
    typeSurf = BASICFONT.render('Type:  %s' % type, True, TEXTCOLOR)
    typeRect = typeSurf.get_rect()
    typeRect.topleft = (WINDOWWIDTH - 625, 80)
    DISPLAYSURF.blit(typeSurf, typeRect)
    ...
    # draw the number of used pieces
    piecesSurf = BASICFONT.render('Pieces:  %s' % used_pieces, True, TEXTCOLOR)
    piecesRect = piecesSurf.get_rect()
    piecesRect.topleft = (WINDOWWIDTH - 150, 120)
    DISPLAYSURF.blit(piecesSurf, piecesRect)


    # draw the Iteration text
    expSurf = BASICFONT.render('Iteration:  %s' % exp, True, TEXTCOLOR)
    expRect = expSurf.get_rect()
    expRect.topleft = (WINDOWWIDTH - 625, 120)
    DISPLAYSURF.blit(expSurf, expRect)

    if gene != '':
        # draw the Generation text
        geneSurf = BASICFONT.render('Generation:  %s' % gene, True, TEXTCOLOR)
        geneRect = expSurf.get_rect()
        geneRect.topleft = (WINDOWWIDTH - 625, 160)
        DISPLAYSURF.blit(geneSurf, geneRect)
